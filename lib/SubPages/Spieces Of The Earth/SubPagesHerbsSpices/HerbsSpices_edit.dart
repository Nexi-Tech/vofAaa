...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: HerbsSpicesEdit('Herbs & Spices_edit'),
    );
  }
}

class HerbsSpicesEdit extends StatefulWidget {
  final String title;

  HerbsSpicesEdit(this.title);

  @override
  _HerbsSpicesEditState createState() => _HerbsSpicesEditState();
}

class _HerbsSpicesEditState extends State<HerbsSpicesEdit> {
  late String Title;
  late CollectionReference HerbsSpicesCollection;
  ...
  void initState() {
    super.initState();
    Title = widget.title;
    HerbsSpicesCollection = FirebaseFirestore.instance.collection('HerbsSpices');
    getPrayers();
  }
  ...
  Future<void> addPrayer(String title, PickedFile imageFile) async {
    try {
      ...
      DocumentReference docRef = await HerbsSpicesCollection.add({
        'title': title,
        'imageUrl': imageUrl,
  ...
  Future<String> uploadImage(String fileName, PickedFile imageFile) async {
    try {
      ...
      Reference ref = FirebaseStorage.instance.ref().child('HSimages/$fileName.jpg');
  ...
  Future<void> getPrayers() async {
    var querySnapshot = await HerbsSpicesCollection.orderBy('createdAt').get();
    setState(() {
  ...
  void deletePrayer(String id) async {
    try {
      await HerbsSpicesCollection.doc(id).delete();
      getPrayers();
  ...
  Future<void> _replaceImage(String imagePathType, int index) async {
    ...
    if (pickedFile != null) {
      ...
      try {
        ...
        await firebase_storage.FirebaseStorage.instance.ref('HSimages/$fileName').putFile(image);
        String downloadURL = await firebase_storage.FirebaseStorage.instance.ref('HSimages/$fileName').getDownloadURL();
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  Future<void> _fetchImageDetails() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('HerbsSpicesDetails').doc(widget.documentId).get();
      if (doc.exists) {
        setState(() {
  ...
  Future<void> _saveChangesToFirestore() async {
    try {
      await FirebaseFirestore.instance.collection('HerbsSpicesDetails').doc(widget.documentId).set({
        'title': editedTitle,
                                  ...
                                  ElevatedButton(
                                    onPressed: () async {
                                      ...
                                      try {
                                        ...
                                        DocumentReference DocRef = FirebaseFirestore.instance.collection('HerbsSpices').doc(widget.documentId);
