...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: HerbsSpices('HerbsSpices'),
    );
  }
}

class HerbsSpices extends StatefulWidget {
  final String title;

  HerbsSpices(this.title);

  @override
  _HerbsSpicesState createState() => _HerbsSpicesState();
}

class _HerbsSpicesState extends State<HerbsSpices> {
  late String Title;
  ...
  void initState() {
    super.initState();
    Title = widget.title;
    prayersCollection = FirebaseFirestore.instance.collection('HerbsSpices');
  }
    ...
    try {
      ...
      if (cachedDetails != null) {
        ...
        DocumentSnapshot doc = await FirebaseFirestore.instance.collection(
            'HerbsSpicesDetails').doc(documentId).get();
        if (doc.exists) {
          setState(() {
            editedTitle = title;
