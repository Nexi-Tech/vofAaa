...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: RecipiesEdit('Recipies_edit'),
    );
  }
}

class RecipiesEdit extends StatefulWidget {
  final String title;

  RecipiesEdit(this.title);

  @override
  _RecipiesEditState createState() => _RecipiesEditState();
}

class _RecipiesEditState extends State<RecipiesEdit> {
  late String Title;
  late CollectionReference RecipiesCollection;
  ...
  void initState() {
    super.initState();
    Title = widget.title;
    RecipiesCollection = FirebaseFirestore.instance.collection('Recipies');
    getPrayers();
  }
  ...
  Future<void> addPrayer(String title, PickedFile imageFile) async {
    try {
      ...
      DocumentReference docRef = await RecipiesCollection.add({
        'title': title,
        'imageUrl': imageUrl,
  ...
  Future<String> uploadImage(String fileName, PickedFile imageFile) async {
    try {
      ...
      Reference ref = FirebaseStorage.instance.ref().child('Simages/$fileName.jpg');
  ...
  Future<void> getPrayers() async {
    var querySnapshot = await RecipiesCollection.orderBy('createdAt').get();
    setState(() {
  ...
  void deletePrayer(String id) async {
    try {
      await RecipiesCollection.doc(id).delete();
      getPrayers();
  ...
  Future<void> _replaceImage(String imagePathType, int index) async {
    ...
    if (pickedFile != null) {
      ...
      try {
        ...
        await firebase_storage.FirebaseStorage.instance.ref('Simages/$fileName').putFile(image);
        String downloadURL = await firebase_storage.FirebaseStorage.instance.ref('Simages/$fileName').getDownloadURL();
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  late String editedStepByStep = ''; // Dodana inicjalizacja
  late String editedIngridients = ''; // Dodana inicjalizacja
  ...
  Future<void> _fetchImageDetails() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('RecipiesDetails').doc(widget.documentId).get();
      if (doc.exists) {
        setState(() {
          editedTitle = doc['title'] ?? ''; // Dodana obsługa null
          editedStepByStep = doc['StepByStep'] ?? ''; // Dodana obsługa null
          editedIngridients = doc['ingridients'] ?? '';
          titleController.text = editedTitle;
          StepByStepController.text = editedStepByStep;
          IngridientsController.text = editedIngridients;
  ...
  Future<void> _saveChangesToFirestore() async {
    try {
      await FirebaseFirestore.instance.collection('RecipiesDetails').doc(widget.documentId).set({
        'title': editedTitle,
        'ingridients': editedIngridients,
        'StepByStep': editedStepByStep,
                                  ...
                                  ElevatedButton(
                                    onPressed: () async {
                                      ...
                                      try {
                                        ...
                                        DocumentReference DocRef = FirebaseFirestore.instance.collection('Recipies').doc(widget.documentId);
