...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: CatholicEdit('Catholic_edit'),
    );
  }
}

class CatholicEdit extends StatefulWidget {
  final String title;

  CatholicEdit(this.title);

  @override
  _CatholicEditState createState() => _CatholicEditState();
}

class _CatholicEditState extends State<CatholicEdit> {
  late String Title;
  late CollectionReference CatholicprayersCollection;
  ...
  void initState() {
    super.initState();
    Title = widget.title;
    CatholicprayersCollection = FirebaseFirestore.instance.collection('Catholic_prayers');
    getPrayers();
  }
  ...
  Future<void> addPrayer(String title, PickedFile imageFile) async {
    try {
      ...
      DocumentReference docRef = await CatholicprayersCollection.add({
        'title': title,
        'imageUrl': imageUrl,
  ...
  Future<String> uploadImage(String fileName, PickedFile imageFile) async {
    try {
      ...
      Reference ref = FirebaseStorage.instance.ref().child('cimages/$fileName.jpg');
  ...
  Future<void> getPrayers() async {
    var querySnapshot = await CatholicprayersCollection.orderBy('createdAt').get();
    setState(() {
  ...
  void deletePrayer(String id) async {
    try {
      await CatholicprayersCollection.doc(id).delete();
      getPrayers();
  ...
  Future<void> _replaceImage(String imagePathType, int index) async {
    ...
    if (pickedFile != null) {
      ...
      try {
        ...
        await firebase_storage.FirebaseStorage.instance.ref('cimages/$fileName').putFile(image);
        String downloadURL = await firebase_storage.FirebaseStorage.instance.ref('cimages/$fileName').getDownloadURL();
...
class _DetailsPageState extends State<DetailsPage> {
  ...
  Future<void> _fetchImageDetails() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('CatholicDetails').doc(widget.documentId).get();
      if (doc.exists) {
        setState(() {
  ...
  Future<void> _saveChangesToFirestore() async {
    try {
      await FirebaseFirestore.instance.collection('CatholicDetails').doc(widget.documentId).set({
        'title': editedTitle,
                                  ...
                                  ElevatedButton(
                                    onPressed: () async {
                                      ...
                                      try {
                                        ...
                                        DocumentReference CatholicprayerDocRef = FirebaseFirestore.instance.collection('Catholic_prayers').doc(widget.documentId);

                                        // Zaktualizowanie tytułu w dokumencie
                                        await CatholicprayerDocRef.update({
                                          'title': newTitle,
                                        });
