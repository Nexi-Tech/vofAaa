...
class Catholic extends StatefulWidget {
  ...
  @override
  _CatholicState createState() => _CatholicState();
}
...
class _CatholicState extends State<Catholic> {
  ...
  void initState() {
    super.initState();
    Title = widget.title;
    prayersCollection = FirebaseFirestore.instance.collection('Catholic_prayers');
  }
  ...
  Future<void> _fetchImageDetailsOffline() async {
    try {
      ...
      if (cachedDetails != null) {
        ...
        DocumentSnapshot doc = await FirebaseFirestore.instance.collection('CatholicDetails').doc(documentId).get();
        if (doc.exists) {
          setState(() {
            editedTitle = title;
