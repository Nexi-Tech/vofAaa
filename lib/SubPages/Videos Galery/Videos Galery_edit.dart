...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: VideosGaleryEdit('Videos Galery Edit'),
    );
  }
}
...
class VideosGaleryEdit extends StatefulWidget {
  final List<VideoModel> _videos = [];
  late String title;

  VideosGaleryEdit(String sTitle) {
    title = sTitle;
  }
  ...
  void saveVideoDetails(
    ...
    try {
      await FirebaseFirestore.instance.collection('videoDetails').doc(documentId).set({
        'videoUrl': videoUrl,
        'title': title,
        'details': details,
  ...
  @override
  _VideosGaleryEditState createState() => _VideosGaleryEditState();
}

class _VideosGaleryEditState extends State<VideosGaleryEdit> {
  Future<void> loadVideosFromFirestore() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('videoDetails').doc('videosData').get();
      if (doc.exists) {
        ...
        doc = await FirebaseFirestore.instance.collection('videoDetails').doc('count').get();
        if (doc.exists) {
          int count = doc['count'];
          setState(() {
  ...
  Future<void> saveVideosToFirestore() async {
    try {
      ...
      await FirebaseFirestore.instance.collection('videoDetails').doc('videosData').set({
        'videos': videosDataList,
      }, SetOptions(merge: true));

      await FirebaseFirestore.instance.collection('videoDetails').doc('count').set({
